...
DJVPoint = namedtuple('DJVPoint', 't x')
...
class Marbles(object):
    def __init__(self):  # , ins_num, mport, key, scale, octave=1, speed=1):
        ...
        for i in range(8):
            self.ring_buffer.set(DJVPoint(randint(-3, 3), 0))
    ...
    def jitter_source(self):
        self.jitter_src = (self.jitter_src + 1) % 100
        jitter_sine_wave = sin(self.jitter_src/10)
        if self.ctl_jitter == 0:
            return 0
        elif self.ctl_jitter < 8:
            jitter = jitter_sine_wave * 3 / self.ctl_jitter
            return jitter
        else:
            jitter = randint(-3, 3)
            return jitter
    ...
    def step_beat(self, global_beat):
        ...
        print('!')
        self.ring_buffer.next()
        next_jitter = self.jitter_src()

        self.local_beat_position = local
        new_notes = []
        self.ring_buffer.set(next_jitter, new_notes)
